...
export class ReservationsDataAccess {
    ...
    public async createReservation(reservation: Omit<Reservation, 'id'> | Reservation){
        const reservationWithId: Reservation = { ...reservation, id: ('id' in reservation ? reservation.id : '') || '' };
        const id = await this.reservationsDataBase.insert(reservationWithId);
        return id;
    }
